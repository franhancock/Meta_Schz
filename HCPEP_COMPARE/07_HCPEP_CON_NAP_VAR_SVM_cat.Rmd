---
title: "Categorization of HCPEP COND based on VAR dynamic metrics - SVM"
author: 
- name: "Fran Hancock"
...
#cv__fit<-train(COND~  VAR4 , data=RegtbTrain, method = "glm", trControl = train.control, family="binomial")
...
pdf(paste("Figures/07_", run, "Cross_validation_HCPEP_CON_NAP_VAR_AUC_PLOT_SVM.pdf"))
par(cex.axis=1.5)
plot(roc, main=paste(run, "HCPEP VAR X-VAL SVM ROC curve for NAP CON"),
     col="blue",
     lwd=3,
     cex.lab = 1.5,
     cex.main = 1.5)
...
# create the datasets seperately for CON and SCHZ so that it is possible to create a random sample of 53 subjects to match the classifier from HCPEP
  ...
  pdf(paste("Figures/07_external_validation_HCPEP_", run,"_VAR_AUC_PLOT_COBRE_SVM.pdf"))
  par(cex.axis=1.5)
  plot(roc, main=paste("VAR SVM HCPEP->COBRE ROC curve"), 
       col="blue", 
       lwd=3,
       cex.lab = 1.5,
       cex.main = 1.5)
  ...
  if (meth=="boot"){
    ...
    write.xlsx(cv_tab,paste("Results/07_",run,"_",  "_Confusion_HCPEP_VAR_COBRE_SVM.xlsx"),"ConfusionMatrix" )
    write.xlsx(toexcel,paste("Results/07_",run,"_", "_Confusion_HCPEP_VAR_COBRE_SVM.xlsx"),"results", append=TRUE)
    write.xlsx(mROCtxt,paste("Results/07_",run,"_", "_Confusion_HCPEP_VAR_COBRE_SVM.xlsx"),"auc", append=TRUE)
  }
