...
run=input(" Which run ? : ");
sub=input("Which subject (1-53) : ");


V1_con=struct2array(load([CON_PROJECT '/RUN' num2str(run) '/LEiDA_EigenVectors.mat'],'V1_all'));
V1_nap=struct2array(load([NAP_PROJECT '/RUN' num2str(run) '/LEiDA_EigenVectors.mat'],'V1_all'));

time_con=struct2array(load([CON_PROJECT '/RUN' num2str(run) '/LEiDA_EigenVectors.mat'],'Time_sessions'));
time_nap=struct2array(load([NAP_PROJECT '/RUN' num2str(run) '/LEiDA_EigenVectors.mat'],'Time_sessions'));

VAR_con=struct2array(load([CON_PROJECT '/RUN' num2str(run) '/LEiDA_KOP_ALL_RUN' num2str(run) '.mat'],'ICN_EV_ALL_MEAN'));
VAR_nap=struct2array(load([NAP_PROJECT '/RUN' num2str(run) '/LEiDA_KOP_ALL_RUN' num2str(run) '.mat'],'ICN_EV_ALL_MEAN'));

EV_con=struct2array(load([CON_PROJECT '/RUN' num2str(run) '/LEiDA_KOP_ALL_RUN' num2str(run) '.mat'],'ICN_EV_ALL_AVG'));
EV_nap=struct2array(load([NAP_PROJECT '/RUN' num2str(run) '/LEiDA_KOP_ALL_RUN' num2str(run) '.mat'],'ICN_EV_ALL_AVG'));

mode_con=struct2array(load([CON_PROJECT '/RUN' num2str(run) '/LEiDA_KOP_ALL_RUN' num2str(run) '.mat'],'STATE_IDX'));
mode_nap=struct2array(load([NAP_PROJECT '/RUN' num2str(run) '/LEiDA_KOP_ALL_RUN' num2str(run) '.mat'],'STATE_IDX'));

kmeans_con=struct2array(load([CON_PROJECT '/RUN' num2str(run) '/LEiDA_Kmeans_results.mat'],'Kmeans_results'));
kmeans_nap=struct2array(load([NAP_PROJECT '/RUN' num2str(run) '/LEiDA_Kmeans_results.mat'],'Kmeans_results'));
...
for t=1:Tslices_con
    ...
    elseif run ==2
        ...
        set(p,'LineStyle','none','FaceColor',cmap(idx_con(t),:));
    end
...
title(['RUN ' num2str(run) ' NAP SUB ' num2str(sub)],'FontSize',18);
...
for t=1:Tslices_nap
    ...
    set(p,'LineStyle','none','FaceColor',cmap(idx_nap(t),:));
end
...
saveas(gcf,[PROJECT 'Figures/EIGS/mode_eigenvectors_run' num2str(run) '_sub_' num2str(sub)],'jpeg')
