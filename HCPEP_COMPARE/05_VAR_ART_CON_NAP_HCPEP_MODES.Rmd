---
title: "Permutation tests for dFC Mode Metrics - VAR HCPEP RAW"
author: "Fran Hancock"
...
write.xlsx(normality,"Results/05_Mode_VAR_PermAss.xlsx","normality")
write.xlsx(equil,"Results/05_Mode_VAR_PermAss.xlsx","equivalence", append=TRUE)
write.xlsx(effSize,"Results/05_Mode_VAR_PermAss.xlsx","effect size", append=TRUE)
...
for( m in mode) {
  ...
  res.art<-art(VAR~COND*RUN + (1|SUB), data=RegtbM)
  res.aov<-anova(res.art)
  
  write.xlsx(res.aov,paste("Results/05_", m, "_VAR.xlsx",sep=""),"AOV")
  ...
  if (res.aov[3,5] > 0.05){
    ...
    write.xlsx(pwc_cond,paste("Results/05_", m,"_VAR.xlsx",sep=""),"PWC_COND", append=TRUE)
    write.xlsx(effsize,paste("Results/05_",m,"_VAR.xlsx",sep=""),"Effect Size", append=TRUE)
   ...
   # Interaction so just run 1-way anova for CON with Friednman test because it is a repeated measure 
    ...
    write.xlsx(sapply(res.fried,unlist),paste("Results/05_",m,"_VAR.xlsx",sep=""),"CON_FRIED", append=TRUE)
    ...
    if (res.fried$p.value < 0.05){
      pwc_run <- RegtbTmp %>%
        ...
        write.xlsx(pwc_run,paste("Results/05_",m,"_VAR.xlsx",sep=""),"CON_PWC_RUN", append=TRUE)
        write.xlsx(effsize,paste("Results/05_",m,"_VAR.xlsx",sep=""),"CON Effect Size", append=TRUE)      
    ...
    print(res.fried)
    write.xlsx(sapply(res.fried,unlist),paste("Results/05_",m,"_VAR.xlsx",sep=""),"NAP_FRIED", append=TRUE)
    res.fried$p.value
    ...
    if (res.fried$p.value < 0.05){
      pwc_run <- RegtbTmp %>%
        ...
        write.xlsx(pwc_run,paste("Results/05_",m,"_VAR.xlsx",sep=""),"NAP_PWC_RUN", append=TRUE)
        write.xlsx(effsize,paste("Results/05_",m,"_VAR.xlsx",sep=""),"NAP Effect Size", append=TRUE)   
    ...
    for(r in runs){
      ...
      write.xlsx(pwc_cond,paste("Results/05_",m,"_VAR.xlsx",sep=""),paste(r,"_PWC_COND"), append=TRUE)
      write.xlsx(effsize,paste("Results/05_",m,"_VAR.xlsx",sep=""),paste(r,"_Effect Size"), append=TRUE) 
