---
title: "Categorization of HCPEP COND based on VAR NM  dynamic metrics"
author: 
- name: "Fran Hancock"
...
DYN_CON<-read.csv('/Users/HDF/Dropbox/Fran/Academics/PhD/matlab/projects/HCPEP_CON_NM/Regtable_modes_RAW.csv')
DYN_NAP<-read.csv('/Users/HDF/Dropbox/Fran/Academics/PhD/matlab/projects/HCPEP_NAP_NM/Regtable_modes_RAW.csv')
...
Mode6VARRuns<-DYN[DYN$MODE %in% c('MODE6'),]
Mode6VAR<-Mode5VARRuns[Mode1VARRuns$RUN %in% c(run),]
Mode7VARRuns<-DYN[DYN$MODE %in% c('MODE7'),]
Mode7VAR<-Mode5VARRuns[Mode1VARRuns$RUN %in% c(run),]
...
Mode25VAR["VAR6"]<-Mode6VAR$VAR
Mode25VAR["VAR7"]<-Mode7VAR$VAR
...
pdf(paste("Figures/07_", run, "Cross_validation_HCPEP_CON_NAP_VAR_AUC_PLOT_NM.pdf"))
par(cex.axis=1.5)
plot(roc, main=paste(run, "HCPEP NM VAR X-VAL ROC curve for NAP CON"),
     col="blue",
     lwd=3,
     cex.lab = 1.5,
     cex.main = 1.5)
...
```{r,lme_model_cobre,echo=FALSE, message=FALSE}
  ...
  DYN_CONT<-read.csv('/Users/HDF/Dropbox/Fran/Academics/PhD/matlab/projects/COBRE_NM/RUN1/Regtable_modes_RAW.csv')
  DYN_SCHZ<-read.csv('/Users/HDF/Dropbox/Fran/Academics/PhD/matlab/projects/COBRE_NM/RUN2/Regtable_modes_RAW.csv')
...
# create the datasets seperately for CON and SCHZ so that it is possible to create a random sample of 53 subjects to match the classifier from HCPEP
  ...
  Mode6VAR<-DYN_CONT[DYN_CONT$MODE %in% c('MODE6'),]
  Mode7VAR<-DYN_CONT[DYN_CONT$MODE %in% c('MODE7'),]
  ...
  Mode6VAR<-DYN_SCHZ[DYN_SCHZ$MODE %in% c('MODE6'),]
  Mode7VAR<-DYN_SCHZ[DYN_SCHZ$MODE %in% c('MODE7'),]
  ...
  pdf(paste("Figures/07_external_validation_HCPEP_", run,"_VAR_AUC_PLOT_COBRE_NM.pdf"))
  par(cex.axis=1.5)
  plot(roc, main=paste("VAR NM HCPEP->COBRE ROC curve"), 
       col="blue", 
       lwd=3,
       cex.lab = 1.5,
       cex.main = 1.5)
  ...
  if (meth=="boot"){
    ...
    write.xlsx(cv_tab,paste("Results/07_",run,"_",  "_Confusion_HCPEP_VAR_COBRE_NM.xlsx"),"ConfusionMatrix" )
    write.xlsx(toexcel,paste("Results/07_",run,"_", "_Confusion_HCPEP_VAR_COBRE_NM.xlsx"),"results", append=TRUE)
    write.xlsx(mROCtxt,paste("Results/07_",run,"_", "_Confusion_HCPEP_VAR_COBRE_NM.xlsx"),"auc", append=TRUE)
  }
