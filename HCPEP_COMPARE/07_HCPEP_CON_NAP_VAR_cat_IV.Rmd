---
title: "Categorization of HCPEP COND based on VAR dynamic metrics - Internal validation"
author: 
- name: "Fran Hancock"
...
auc_txt
# 
# pdf(paste("Figures/07_", run, "Cross_validation_HCPEP_CON_NAP_VAR_AUC_PLOT.pdf"))
# par(cex.axis=1.5)
# plot(roc, main=paste(run, "HCPEP VAR X-VAL ROC curve for NAP CON"),
#      col="blue",
#      lwd=3,
#      cex.lab = 1.5,
#      cex.main = 1.5)
...
Mode1VARRuns<-DYN[DYN$MODE %in% c('MODE1'),]
Mode1VAR<-Mode1VARRuns[Mode1VARRuns$RUN %in% c(testrun),]
Mode2VARRuns<-DYN[DYN$MODE %in% c('MODE2'),]
Mode2VAR<-Mode2VARRuns[Mode1VARRuns$RUN %in% c(testrun),]
Mode3VARRuns<-DYN[DYN$MODE %in% c('MODE3'),]
Mode3VAR<-Mode3VARRuns[Mode1VARRuns$RUN %in% c(testrun),]
Mode4VARRuns<-DYN[DYN$MODE %in% c('MODE4'),]
Mode4VAR<-Mode4VARRuns[Mode1VARRuns$RUN %in% c(testrun),]
Mode5VARRuns<-DYN[DYN$MODE %in% c('MODE5'),]
Mode5VAR<-Mode5VARRuns[Mode1VARRuns$RUN %in% c(testrun),]
...
RegtbTest<-Mode25VAR
RegtbTest$COND <- factor(RegtbTrain$COND, levels = c("CON","NAP"))
...
#   RegtbTest2$COND <- factor(RegtbTest2$COND, levels = c("CON","NAP"))
  ...
  pt<-prop.table(table(predict(cv_fit$finalModel,RegtbTest)$class ,RegtbTest$COND))
  ...
  pred_prob<-predict(cv_fit$finalModel, RegtbTest, type ="prob")
  pred<-ROCR::prediction(predictions=pred_prob$posterior[, 2], labels=RegtbTest$COND) 
  roc<-performance(pred, measure="tpr", x.measure="fpr")
  ...
  pdf(paste("Figures/07_external_validation_HCPEP_", run,"_VAR_AUC_PLOT_COBRE_IV.pdf"))
  par(cex.axis=1.5)
  plot(roc, main=paste("VAR UP HCPEP RUN2->RUN1 ROC curve"), 
       col="blue", 
       lwd=3,
       cex.lab = 1.5,
       cex.main = 1.5)
  ...
  cv_tab<-table(predict(cv_fit$finalModel,RegtbTest)$class,RegtbTest$COND)
  ...
  if (meth=="boot"){
    ...
    write.xlsx(cv_tab,paste("Results/07_",run,"_",  "_Confusion_HCPEP_VAR_COBRE_IV.xlsx"),"ConfusionMatrix" )
    write.xlsx(toexcel,paste("Results/07_",run,"_", "_Confusion_HCPEP_VAR_COBRE_IV.xlsx"),"results", append=TRUE)
    write.xlsx(mROCtxt,paste("Results/07_",run,"_", "_Confusion_HCPEP_VAR_COBRE_IV.xlsx"),"auc", append=TRUE)
  }
