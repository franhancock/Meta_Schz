...
opts_knit$set(root.dir ='/Users/HDF/Dropbox/Fran/Academics/PhD/matlab/projects/IGNIT_HCPEP/')
...
Regtb <- read.csv('/Users/HDF/Dropbox/Fran/Academics/PhD/matlab/projects/IGNIT_HCPEP/IGNIT_Regtable_RAW.csv',stringsAsFactors=TRUE)
...
RegtbCON<-Regtb[Regtb$COND %in% c("CON"),]
RegtbSCHZ<-Regtb[Regtb$COND %in% c("NAP"),]
...
lmCON<-lm(Kcon~VAR, data=RegtbCON)

sumCON<-summary(lmCON)
print("K con")
sumCON
tidylm<-broom::glance(lmCON)
write.xlsx(tidylm,"Results/01_Regression.xlsx","K_CON")
...
lmSCHZ<-lm(Kschz~VAR, data=RegtbSCHZ)
sumSCHZ<-summary(lmSCHZ)
print("K SCHZ")
sumSCHZ
tidylm<-broom::glance(lmSCHZ)
write.xlsx(tidylm,"Results/01_Regression.xlsx","K_SCHZ", append=TRUE)

R2<-format(signif(sumSCHZ$r.squared,2),digits=2,nsmall=1)
R2adj<-format(signif(sumSCHZ$adj.r.squared,2), digits=2,nsmall=1)

SCHZtxt<-paste("NAP: R2=", R2, ", R2adj=", R2adj,sep="")


pkelso<-ggplot(data=Regtb,aes(x=VAR,y=K,col=COND))+
...
pkelso
ggsave('Figures/01_SCOTT_HCPEP.pdf', width=20, height=20, units="cm")

lmCON<-lm(GINT~VAR, data=RegtbCON)
sumCON<-summary(lmCON)
print(" GINT CON")
sumCON

tidylm<-broom::glance(lmCON)
write.xlsx(tidylm,"Results/01_Regression.xlsx","GINT_CON", append=TRUE)
...
lmSCHZ<-lm(GINT~VAR, data=RegtbSCHZ)
sumSCHZ<-summary(lmSCHZ)
print("GINT SCHZ")
sumSCHZ

tidylm<-broom::glance(lmSCHZ)
write.xlsx(tidylm,"Results/01_Regression.xlsx","GINT_SCHZ", append=TRUE)
...
pint<-ggplot(data=Regtb,aes(x=VAR,y=GINT,col=COND))+
  geom_point(aes(color=COND))+
  geom_smooth(method = "lm", se = FALSE)+
  scale_color_manual("COND", values=setNames(colors,levels(Regtb$COND)))+
  scale_fill_manual("COND", values=setNames(colors,levels(Regtb$COND)))+
...
lmCON<-lm(FSEG~VAR, data=RegtbCON)
sumCON<-summary(lmCON)
print("FSEG CON")
sumCON

tidylm<-broom::glance(lmCON)
write.xlsx(tidylm,"Results/01_Regression.xlsx","FSEG_CON", append=TRUE)
...
lmSCHZ<-lm(FSEG~VAR, data=RegtbSCHZ)
sumSCHZ<-summary(lmSCHZ)
print("FSEG SCHZ")
sumSCHZ

tidylm<-broom::glance(lmSCHZ)
write.xlsx(tidylm,"Results/01_Regression.xlsx","FSEG_SCHZ", append=TRUE)
...
pseg<-ggplot(data=Regtb,aes(x=VAR,y=FSEG,col=COND))+
  geom_point(aes(color=COND))+
  geom_smooth(method = "lm", se = FALSE)+
  scale_color_manual("COND", values=setNames(colors,levels(Regtb$COND)))+
  scale_fill_manual("COND", values=setNames(colors,levels(Regtb$COND)))+
...
ggsave('Figures/01_INT_SEG_HCPEP.pdf', width=60, height=20, units="cm")


# 
# 
# # now get a plot of GINT and FSEG against VAR for each condition
# 
...
# 
# pint<-ggplot(data=RegtbCONall,aes(x=VAR,y=VOI,col=METRIC))+
...
# 
# pint<-ggplot(data=RegtbSCHZall,aes(x=VAR,y=VOI,col=METRIC))+
#             geom_point(aes(color=METRIC))+
#             geom_smooth(method = "lm", se = FALSE)
# pint
...
write.xlsx(normality,"Results/01_Global_GINT_PermAss.xlsx","normality")
write.xlsx(equil,"Results/01_Global_GINT_PermAss.xlsx","equivalence", append=TRUE)
write.xlsx(effSize,"Results/01_Global_GINT_PermAss.xlsx","effect size", append=TRUE)
...
stat.test["group2"]<-"NAP"
#

write.xlsx(stat.test,"Results/01_Global_GINT_perm_results.xlsx")
...
write.xlsx(as_tibble(GINT_se),"Results/01_Global_GINT_descriptive_stats.xlsx",desc)
...
write.xlsx(normality,"Results/01_Global_FSEG_PermAss.xlsx","normality")
write.xlsx(equil,"Results/01_Global_FSEG_PermAss.xlsx","equivalence", append=TRUE)
write.xlsx(effSize,"Results/01_Global_FSEG_PermAss.xlsx","effect size", append=TRUE)
...
write.xlsx(stat.test,"Results/01_Global_FSEG_perm_results.xlsx")
...
write.xlsx(as_tibble(FSEG_se),"Results/01_Global_FSEG_descriptive_stats.xlsx",desc)
...
pdf("Figures/01_Global_perm_FSEG.pdf",6,4)
barplt
dev.off()
