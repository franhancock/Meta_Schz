---
title:  "Permutation testing  for dFC Metrics - META  HCPEP"
author: "Fran Hancock"
...
R2<-format(round(sumCON$r.squared,2),digits=2,nsmall=1)
R2adj<-format(round(sumCON$adj.r.squared,2), digits=2,nsmall=1)
...
R2<-format(round(sumSCHZ$r.squared,2),digits=2,nsmall=1)
R2adj<-format(round(sumSCHZ$adj.r.squared,2), digits=2,nsmall=1)
...
ggsave('Figures/01_INT_SEG_HCPEP_META.pdf', width=60, height=20, units="cm")
...
# code adapted from add_roundicance 
...
stat.test<-data.frame(s_df,s_test, s_perm_p, s_upci,s_loci, s_mean_con, s_mean_nap,p.round)
...
barplt<-bp +stat_pvalue_manual(
  stat.test, y.position = c(0.25), label = "p.round", tip.length = 0,hide.ns=FALSE,size=8, color="red",vjust=0.5)
pdf("Figures/01_Global_perm_GINT_META.pdf",6,4)
barplt
dev.off()
...
barplt<-bp +stat_pvalue_manual(
  stat.test, y.position = c(0.0065), label = "p.round", tip.length = 0,hide.ns=FALSE,size=8, color="red")
