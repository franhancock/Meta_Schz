...
write.xlsx(normality,"Results/02_Mode_META_PermAss.xlsx","normality")
write.xlsx(equil,"Results/02_Mode_META_PermAss.xlsx","equivalence", append=TRUE)
write.xlsx(effSize,"Results/02_Mode_META_PermAss.xlsx","effect size", append=TRUE)
...
for( m in mode) {
  ...
  stat.test_list<-perm.t.test(xx$META,yy$META, paired=FALSE) 
  ...
  if(mm==1){
     write.xlsx(as_tibble(stat.test),"Results/02_Mode_META_perm_results.xlsx",htest)
  } else {
    write.xlsx(as_tibble(stat.test),"Results/02_Mode_META_perm_results.xlsx",htest, append=TRUE)
  }
  
  Regtb_tmp<-Regtb[Regtb$MODE %in% c(m),]
  
  META_means<-Regtb_tmp %>%
    group_by(COND) %>%
  ...
  desc<-paste("Descp",m)
  if(mm==1){
     write.xlsx(as_tibble(META_se),"Results/02_Mode_META_descriptive_stats.xlsx",desc)
  } else {
    write.xlsx(as_tibble(META_se),"Results/02_Mode_META_descriptive_stats.xlsx",desc, append=TRUE)
  }
  ...
  barplt<-bp +stat_pvalue_manual(
    stat.test, y.position = c(0.22), label = "p.signif", tip.length = 0,hide.ns=FALSE,size=8, color="red")
  switch(mm,
         bp1<-barplt,
         bp2<-barplt,
         bp3<-barplt,
...
ggsave('Figures/02_perm_META_CONNAP_mode_bar.pdf', width=60, height=20, units="cm")
